...
if [ $num -eq 1 ]; then
    ...
    mkdir -p /home/$name1
    rclone mount $name1: /home/$name1 --allow-other --allow-non-empty --vfs-cache-mode writes &
    sleep 5 && df -h
elif [ $num -eq 3 ]; then
    ...
    read -p "请输入文件夹路径：" dirpath
    filename="${dirpath##*/}"
    nohup zip -r /home/aabc/compression/$filename.zip $dirpath &>/dev/null &
elif [ $num -eq 7 ]; then
    echo -e "\e[1;33m注意只会批量压缩指定目录下的一级子文件夹，二级以后不会压缩，压缩前请将文件夹整理规范。\e[0m"
    read -p "请输入文件夹路径：" dirpath
    nohup ls -Al $dirpath | grep "^d" | awk '{print $9}' | xargs -i zip -r /home/aabc/compression/{}.zip $dirpath/{} &>/dev/null &
elif [ $num -eq 8 ]; then
