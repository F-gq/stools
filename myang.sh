...
if [ -e /root/tg ];then
    echo -e "\e[1;32mtelegram-cli已安装.\e[0m"
else
    https://github.com/vysheng/tg.git && cd tg && ./configure --disable-liblua && make -j8
fi
...
\mv -f /root/AutoRclone/aria2.conf /root/.aria2c/aria2.conf
\mv -f /root/AutoRclone/tg-server.pub /root/tg/tg-server.pub                                      #替换aria2配置文件
